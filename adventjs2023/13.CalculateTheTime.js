...
function calculateTime(deliveries) {
    ...
    const totalDeliveryTime = deliveries.reduce((total, time) => {
        const [hours, minutes, seconds] = time.split(':').map(Number);
        return total + hours * 3600 + minutes * 60 + seconds;
    }, 0);

    const remainingTime = totalDeliveryTime - timeLimit;

    const hours = String(Math.abs(Math.floor(remainingTime / 3600))).padStart(2, '0');
    const minutes = String(Math.abs(Math.floor((remainingTime % 3600) / 60))).padStart(2, '0');
    const seconds = String(Math.abs(remainingTime % 60)).padStart(2, '0');

    return (remainingTime < 0 ? '-' : '') + `${hours}:${minutes}:${seconds}`;
}
